...
async function complete_coversation(api_key, context, conversation, guide) {
  ...
  try {
    let res = await bilbil_fetch("https://api.openai.com/v1/chat/completions", {
      method: "POST",
      headers: {
...
function setMainBotChat() {
  ...
  bilbil_fetch(`https://api.telegram.org/bot${tbot}/getUpdates`).then(
    ({ data }) => {
